...
float fov = (40.0f * ALTO) / 600.0f, aspect = 4.0f / 3.0f; // Proporcional al alto de la ventana original 
...
void render_scene()
{
	...
	T = glm::translate(glm::vec3(3.0f * cos(t), 3.0f * sin(t), 0.0f)); // T.circular
...
void ResizeCallback(GLFWwindow* window, int width, int height)
{
	...
	aspect = static_cast<float>(ANCHO) / static_cast<float>(ALTO);
}
...
static void KeyCallback(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	{
		pos_obs.x -= 0.1f;  // Acercar la cámara al origen en el eje X
		printf("Nueva posición del observador: X=%.2f, Y=%.2f, Z=%.2f\n", pos_obs.x, pos_obs.y, pos_obs.z);
	}

	if (key == GLFW_KEY_DOWN && action == GLFW_PRESS)
	{
		pos_obs.x += 0.1f;  // Alejar la cámara del origen en el eje X
		printf("Nueva posición del observador: X=%.2f, Y=%.2f, Z=%.2f\n", pos_obs.x, pos_obs.y, pos_obs.z);
	}
